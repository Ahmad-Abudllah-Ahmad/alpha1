...
export default function EstimationWorkspace() {
  ...
  if (view.kind === "floor") {
    ...
    return (
      <ViewFade viewKey={`floor-${view.projectId}-${view.floorId}`} variant="tab" className="flex h-full min-h-0 flex-1 flex-col">
        <div className="flex h-full min-h-0 flex-1 flex-col">
          <div className="flex shrink-0 items-center gap-2 border-b border-border/50 bg-background px-3 py-1.5">
            <Button
              variant="ghost"
              size="sm"
          ...
          <OpenTakeoffEmbed className="relative min-h-0 flex-1 overflow-hidden bg-[#dbe3e6] dark:bg-[#1a2332]" />
        </div>
      </ViewFade>
    );
  }
  ...
  if (view.kind === "project") {
    return (
      <ViewFade viewKey={`project-${view.projectId}`} variant="tab" className="flex h-full min-h-0 flex-1 flex-col">
        <ProjectDetailView
          store={store}
